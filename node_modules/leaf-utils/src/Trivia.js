...
module.exports = async (options) => {
  try {
	...
	if (!options.embed.footer) {
		options.embed.footer = '©️ Leaf-utils';
	}
	if (typeof options.embed.footer !== 'string') {
		throw new TypeError('Leaf-utils: embed footer must be a string.');
	}

	if (!options.embed.timestamp) options.embed.timestamp = true;
	if (typeof options.embed.timestamp !== 'boolean') {
		throw new TypeError('Leaf-utils: timestamp must be a boolean.');
	}
	...
	if (typeof options.difficulty !== 'string') {
		throw new TypeError('Leaf-utils: difficulty must be a string.');
	}

	if (!options.thinkMessage) options.thinkMessage = 'I am thinking';
	if (typeof options.thinkMessage !== 'string') {
		throw new TypeError('Leaf-utils: thinkMessage must be a boolean.');
	}
	...
	if (typeof options.winMessage !== 'string') {
		throw new TypeError('Leaf-utils: winMessage must be a boolean.');
	}
	...
	if (typeof options.loseMessage !== 'string') {
		throw new TypeError('Leaf-utils: loseMessage must be a boolean.');
	}
	...
	if (typeof options.emojis.one !== 'string') {
		throw new TypeError('Leaf-utils: emoji one must be an emoji.');
	}

	if (!options.emojis.two) options.emojis.two = '2️⃣';
	if (typeof options.emojis.two !== 'string') {
		throw new TypeError('Leaf-utils: emoji two must be an emoji.');
	}

	if (!options.emojis.three) options.emojis.three = '3️⃣';
	if (typeof options.emojis.three !== 'string') {
		throw new TypeError('Leaf-utils: emoji three must be an emoji.');
	}

	if (!options.emojis.four) options.emojis.four = '4️⃣';
	if (typeof options.emojis.four !== 'string') {
		throw new TypeError('Leaf-utils: emoji four must be an emoji.');
	}

	if (!options.time) options.time = 60000;
	if (parseInt(options.time) < 10000) {
		throw new Error(
			'Leaf-utils: time argument must be greater than 10 Seconds (in ms i.e. 10000).',
		);
	}
	...
	if (typeof options.returnWinner !== 'boolean') {
		throw new TypeError('Leaf-utils: buttonText must be a boolean.');
	}
	...
	gameCollector.on('collect', async (trivia) => {
		...
		if (trivia.customId === winningID) {
			...
			if (options.returnWinner) {
				if (!options.gameID) {
					throw new Error('Leaf-utils: gameID argument was not specified.');
				}
				if (typeof options.gameID !== 'string') {
					throw new TypeError('Leaf-utils: gameID must be a string.');
				}
			}
			if (winningID === id1) {
	...
	gameCollector.on('end', (trivia, reason) => {
        ...
        console.log(`Error Occured | Trivia | Error: ${err.stack}`)
    }
};